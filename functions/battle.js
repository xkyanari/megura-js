...
const attack = async (interaction, player1, player2, criticalHitRate) => {
  try {
    ...
    let message = await interaction.channel.send(
      `${player1.playerName} attacks ${player2.playerName} for ${Math.round(
        finalDamage
      )} damage${isCriticalHit ? " (critical hit)!" : ""}. ${
        player2.playerName
      } has ${Math.max(0, Math.round(remainingHealth))} health remaining.`
    );

    if (Math.max(0, Math.round(remainingHealth)) === 0) {
      await message.edit(
        `${player2.playerName} dodged the final attack and gave up.`
      );
    }
