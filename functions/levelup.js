...
module.exports = async (interaction, level) => {
    try {
        const player = await Player.findOne({ where: { discordID: interaction.member.id, guildID: interaction.guild.id } });
        ...
        context.fillText(`Level Up!`, canvas.width - context.measureText(`Level Up!`).width - 50, canvas.height / 3.25);
        ...
        const { body } = await request(interaction.member.displayAvatarURL({ extension: 'png', size: 512 }));
