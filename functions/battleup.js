...
module.exports = async (interaction, player1, player2) => {
    try {
        ...
        let { body: body1 } = await request(
            player1.user.displayAvatarURL(),
        );
        let chunks1 = [];
        for await (let chunk of body1) {
            chunks1.push(chunk);
        }
        let arrayBuffer1 = Buffer.concat(chunks1);
        let avatar1 = await loadImage(arrayBuffer1);

        let imageX1 = 129.5 - 64.5;
        let imageY1 = 135.5 - 64.5;
        ...
        let arrayBuffer2 = Buffer.concat(chunks2);
        let avatar2 = await loadImage(arrayBuffer2);

        let imageX2 = 347.5 - 64.5;
        let imageY2 = 135.5 - 64.5;
