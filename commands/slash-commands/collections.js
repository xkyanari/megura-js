...
const createEmbedPages = async () => {
    try {
        ...
        const totalPageCount = Object.values(contractsByBlockchain).reduce((total, contracts) => {
            return total + Math.ceil(contracts.length / 10);
        }, 0);
        ...
        for (const blockchain in contractsByBlockchain) {
            ...
            const pages = contractGroups.map((contractsGroup, index) => {
                ...
                const embed = new EmbedBuilder()
                    .setTitle(`Supported Collections (${blockchainName}) - Page ${index + 1}`)
                    .setFooter({ text: `Page ${pageNumber} / ${totalPageCount}` });
    
                contractsGroup.forEach(contract => {
                    embed.addFields({ name: `🔸${contract.contractName}`, value: contract.collectionURL, inline: false });
                });
