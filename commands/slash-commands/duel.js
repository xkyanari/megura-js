  ...
  async execute(interaction) {
    ...
    if (player1.id === player2.id)
      return interaction.editReply(
        "There is a saying that goes:```“The attempt to force human beings to despise themselves is what I call hell.” ― Andre Malraux```Sorry, I cannot allow that."
      );
    if (interaction.client.user.id === player2.id)
      return interaction.editReply("I don't engage in battles.");
    if (player2.user.bot)
      return interaction.editReply("You cannot duel with bots.");
    ...
    try {
      ...
      if (players.length === 2) {
        // checking rank
        if (updatedPlayer2.totalHealth - updatedPlayer1.totalHealth >= 5000)
          return interaction.editReply(
            "Your rank is too low to fight this player."
          );
        if (updatedPlayer2.totalHealth - updatedPlayer1.totalHealth <= -5000)
          return interaction.editReply(
            "Your rank is too high to fight this player."
          );

        // checking balance
        if (updatedPlayer1.iura.walletAmount < 100)
          return interaction.editReply(
            "You do not have sufficient balance to duel! Please carry at least $100 IURA first."
          );
        if (updatedPlayer2.iura.walletAmount < 100)
          return interaction.editReply(
            "This player does not have enough balance to be attacked."
          );
        ...
        if (winner === updatedPlayer1) {
          ...
          await Iura.decrement(
            { walletAmount: totalIura1 },
          ...
          await updatedPlayer1.increment({
            iuraEarned: totalIura1,
          ...
          if (updatedPlayer1.expGained > expPoints(updatedPlayer1.level)) {
            ...
            await interaction.channel.send(
              `\`${updatedPlayer1.playerName}\`, you have leveled up to **${levelUp.level}**!`
            );
          }
        ...
        await interaction.editReply(
          `${player1.user.tag} do not have a voyager profile yet.`
        );
      } else if (!updatedPlayer2) {
        await interaction.editReply(
          `${player2.user.tag} does not have a voyager profile yet.`
        );
      } else {
        await interaction.editReply(
          `Both players do not have voyager profiles yet.`
        );
      }
    } catch (error) {
      console.log(error);
