        ...
        do {
          ...
          if (player_name.content.length <= 20) {
            await interaction.followUp({
              content: `\`${player_name.content}\`, right? Y/N`,
              ephemeral: true,
            });
            let message = await channel.awaitMessages({
              filter,
              max: 1,
              time: 120_000,
              errors: ["time"],
            ...
            if (confirm.content === "Y" || confirm.content === "y") {
              await interaction.followUp({
                content: `Thank you, \`${player_name.content}\`. That's a good name!`,
                ephemeral: true,
              });
              // creates a player profile in the db
              const create_profile = await Player.create({
                guildID: guild.id,
                discordID: member.id,
              ...
              await create_profile.createIura({
                walletName: player_name.content,
                bankName: player_name.content,
              });
              ...
              await wait(4000);
              await interaction.followUp({
                content: `For now, you will travel to the \`past\`. Please take care, \`${player_name.content}\`.`,
                ephemeral: true,
              });
              break;
            } else if (
          ...
          if (attempt == 2) {
            await interaction.followUp({
              content: "Sorry, please come back when you're ready.",
              ephemeral: true,
            });
          }
        } while (attempt < 2);
