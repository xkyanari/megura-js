      ...
      try {
        try {
          ...
          const nftResult = await fetch(
            `https://pro-api.solscan.io/v1.0/nft/wallet/list_nft/${getWallet}?history=false`,
            options
          );
          ...
          contractList.forEach((thisCollection) => {
            const foundItem = nfts.data.list_nfts.find(
              (item) => item.nft_collection_id === thisCollection
            );
            if (foundItem) {
          ...
          collectionList.forEach((thisCollection) => {
            const foundItem = nfts.find(
              (item) => item.collection === thisCollection
            );
            if (foundItem) {
