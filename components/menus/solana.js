...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			const { contractName, traitWeapon, traitArmor } = await Contract.findOne({
			...
			try {
				...
				const nftData = await fetch(
					`https://api-mainnet.magiceden.dev/v2/tokens/${getAddress.mintAddress}`,
					options,
				);
				const nftInfo = await nftData.json();

				let imageURL;
				if (nftInfo.image.startsWith('ipfs://')) {
					imageURL = nftInfo.image.replace('ipfs://', 'https://ipfs.io/ipfs/');
				}
				else {
				...
				const weaponValue =
					nftInfo.attributes.find((item) => item.trait_type === traitWeapon)?.value ?? weapon;
				const armorValue =
					nftInfo.attributes.find((item) => item.trait_type === traitArmor)?.value ?? armor;
