...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!userData) {
            return interaction.reply({ content: `No user data found. Did it time out?`, components: [button], ephemeral: true });
        }
        ...
        if (!player) {
            return interaction.reply({ content: `No player found. Did it time out?`, components: [button], ephemeral: true });
        }
        ...
        if (player.linked === true) {
            await player.save();
            await interaction.reply({ content: `Your NFT has been linked successfully!`, components: [button], ephemeral: true });
        } else {
