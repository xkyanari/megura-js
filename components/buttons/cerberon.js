...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const player = await Player.findOne({ where: { discordID: member.id, guildID: interaction.guild.id }});
        ...
        const margaretha = await interaction.guild.roles.cache.get(guild.margarethaID);
        const cerberon = await interaction.guild.roles.cache.get(guild.cerberonID);
            ...
            try {
                ...
                if (member.roles.cache.some(role => role.name === margaretha.name)) {
                    ...
                    embed.setDescription(`Removed \`${margaretha.name}\`.\nAdded \`${cerberon.name}\`.`);
